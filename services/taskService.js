...
function queryTaskList(req, res, next) {
  ...
  if (!err.isEmpty()) {
    ...
    let query = `select d.id, d.title, d.content, d.status, d.is_major, d.e, d.gmt_expire from task d`;
    querySql(query).then((data) => {
      ...
      if (!data || data.length === 0) {
        ...
        // 拼接分页的sql语句命令
        if (status) {
          let query_1 = `select d.id, d.title, d.content, d.status, d.is_major, d.gmt_create, d.gmt_expire from task d where status='${status}' order by d.gmt_create desc`;
  ...
  if (!err.isEmpty()) {
    ...
    findTask(title, 1).then((task) => {
      if (task) {
        ...
        const query = `insert into task(title, content, status, is_major, gmt_expire) values('${title}', '${content}', 0, 0, '${gmt_expire}')`;
        querySql(query).then((data) => {
...
function editTask(req, res, next) {
  ...
  if (!err.isEmpty()) {
    ...
    findTask(id, 2).then((task) => {
      if (task) {
        const query = `update task set title='${title}', content='${content}', gmt_expire='${gmt_expire}' where id='${id}'`;
        querySql(query).then((data) => {
...
function updateTaskStatus(req, res, next) {
  ...
  if (!err.isEmpty()) {
    ...
    findTask(id, 2).then((task) => {
      if (task) {
        const query = `update task set status='${status}' where id='${id}'`;
        querySql(query).then((data) => {
...
function updateMark(req, res, next) {
  ...
  if (!err.isEmpty()) {
    ...
    findTask(id, 2).then((task) => {
      if (task) {
        const query = `update task set is_major='${is_major}' where id='${id}'`;
        querySql(query).then((data) => {
...
function findTask(param, type) {
  ...
  if (type == 1) {
    // 1:添加类型 2:编辑或删除类型
    query = `select id, title from task where title='${param}'`;
  } else {
    query = `select id, title from task where id='${param}'`;
  }
